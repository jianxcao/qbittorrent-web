...
watch(
  ...
  (newScrollLeft) => {
    if (headerRef.value) {
      const wrapper = headerRef.value.querySelector('.torrent-table-header-cell-wrapper') as HTMLElement
      if (wrapper) {
        wrapper.style.transform = `translateX(-${newScrollLeft}px)`
      }
    }
  }
)
...
function onCheckboxChange(checked: boolean) {
  if (checked) {
    torrentStore.setSelectedKeys(torrentStore.filterTorrents.map((t) => t.hash))
  } else {
